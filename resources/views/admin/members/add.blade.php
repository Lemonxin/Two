...
<body>
        ...
        <form action="" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data">
            ...
            <div class="row cl">
				...
				<select class="select" size="1" name="country_id">
					<option value="" selected>国家/地区</option>
				...
				<select class="select" size="1" name="province_id">
					<option value="" selected>省份/州/直辖市</option>
				</select>
				</span> 
				<span class="select-box" style="width: 110px">
				<select class="select" size="1" name="city_id">
					<option value="" selected>城市/区</option>
				</select>
				</span> 
				<span class="select-box" style="width: 110px">
    ...
    // response表示ajax返回值，如果是json格式可以直接使用
    uploader.on( 'uploadSuccess', function( file,response ) {
        ...
        if(response.errorCode == '0'){
            ...
            layer.mag(response.message,{icon:2});
        }
    ...
    $(function(){
        ...
        $('select[name=country_id]').change(function(){
            ...
            // 发送ajax请求获取该国家的下属内容 （pid = country_id）
            $.get('/admin/members/getAreasByPid',{pid:country_id},function(data){
                // 获取到数据之后将数据组装回页面
                // 循环得到全部需要追加的option
                var ops = '';
        ...
        $('select[name=province_id]').change(function(){
            ...
            // 发送ajax请求获取该国家下属的内容（pid = country_id）
            $.get('/admin/members/getAreasByPid',{pid:province_id},function(data){
                // 获取到数据之后将数据组装会页面
                //循环得到全部需追加的option
                var ops = '';
                $.each(data,function(key,value){
        ...
        $('select[name=city_id]').change(function(){
            ...
            $.get('/admin/members/getAreasByPid',{pid:city_id},function(data){
                // 获取到数据之后将数据组装回页面
                //循环得到全部需追加的option
                var ops = '';
                $.each(data,function(key,value){
